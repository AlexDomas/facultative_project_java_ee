...
import by.bntu.poisit.library_ee.dao.interfaces.JournalDaoInterface;
import by.bntu.poisit.library_ee.dao.connectionpool.*;
...
public class JournalDao extends AbstractDao<Integer, Journal> implements JournalDaoInterface {
    private static final String SQL_INSERT="INSERT INTO Journal (id, mark, note, id_course, id_student) VALUES (NULL, NULL, NULL, ?, ?);";
    ...
    private static final String SQL_FIND_JOURNAL_BY_COURSE_ID="SELECT Journal.id, Journal.mark, Journal.note, Journal.id_course, Journal.id_student, User.first_name, User.last_name FROM Journal LEFT JOIN User ON Journal.id_student = User.id WHERE Journal.id_course=? ORDER BY User.last_name, User.first_name";
    private static final String SQL_FIND_BY_PK="SELECT Journal.id, Journal.mark, Journal.note, Journal.id_course, Journal.id_student, User.first_name, User.last_name FROM Journal LEFT JOIN User ON Journal.id_student = User.id WHERE Journal.id=?";
    private static final String SQL_UPDATE="UPDATE Journal SET mark=?, note=?, id_course=?, id_student=? WHERE id=?";
    ...
    public Journal update(Journal entity) throws DaoException{
        ...
        try {
            ...
            try {
                ...
                stmt.setInt(3, entity.getCourseId());
                stmt.setInt(4, entity.getStudentId());
